...
class Heater(object): # Junkers Euromaxx for now.
    ...
    def __init__(self, svc_hmode='GSS', svc_Gtemp='TGW',svc_Htemp='THW', 
    ...
    def read_svc(self):
        ...
        self.GSW = (d.get_divalues('self.svc_hmode'))
        self.TGW = ac.get_aivalues('self.svc_Gtemp') # water from gasheater - actual on, actual ret, setpoint, hilim
        self.THW = ac.get_aivalues('self.svc_Htemp') # water to floors -  actual on, actual ret, setpoint, hilim
        self.KGPW = ac.get_aivalues('self.svc_P') # kP for loops G, H
        self.KGIW = ac.get_aivalues('self.svc_I') # kI for loops G, H
        self.KGDW = ac.get_aivalues('self.svc_D') # kD for loops G, H 
    ...
    def write_svcs(self):
        ...
        ac.set_aivalues('self.svc_pwm', values = pwm_values)
        ac.set_aivalues('self.svc_Gdebug', values=[self.val2int(tempvarsG['error'],10), self.val2int(tempvarsG['outP'],10), self.val2int(tempvarsG['outI'],10), self.val2int(tempvarsG['outD'],10) ]) # out comp x 10 for loop 0
        ac.set_aivalues('self.svc_Hdebug', values=[self.val2int(tempvarsH['error'],10), self.val2int(tempvarsH['outP'],10), self.val2int(tempvarsH['outI'],10), self.val2int(tempvarsH['outD'],10) ]) # PID comp for loop 1
        ac.set_aivalues('self.svc_noint', values=[tempvarsG['extnoint'], tempvarsH['extnoint'] ]) # ext int stop
    ...
    def output(self):
        ...
        try:
            ...
            self.pwm_values = [ self.val2int(self.pid_gas[0].output(TGW[2],TGW[0],noint=noint)), self.val2int(self.pid_gas[1].output(THW[2],THW[0], noint=noint)) ]
