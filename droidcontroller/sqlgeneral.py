...
try:
    ...
    for row in cur:
        if ':' in row[1]: # over tcp
            mb.append(CommModbus(host=row[1].split(':')[0], port=int(row[1].split(':')[1]), name='mbi'+str(row[0]))) # modbustcp over tcp
            ...
            mb.append(CommModbus(host=row[1].split('|')[0], speed=int(row[1].split('|')[1]), parity = parity, name='mbi'+str(row[0]))) # modbustcp over tcp
