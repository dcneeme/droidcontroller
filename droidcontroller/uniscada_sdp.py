...
# able to restore unsent history from buffer2server.sql when connecctivity restored
# FIXME: trying to use sdp, incl sha256 signature
...
class UDPchannel():
    ...
    def __init__(self, id ='000000000000', ip='127.0.0.1', port=44445, receive_timeout=0.1, retrysend_delay=5, loghost='0.0.0.0', logport=514, copynotifier=None, secret_key = None): # delays in seconds
        ...
        try:
            from uniscada.sdp import SDP
            #self.sdp = SDP(self.secret_key)
        except:
            log.warning('sdp not imported!') 
    ...
    def unsent(self, maxage=86400):   # 24 hours max history to be kept
        ...
        try:
            ...
            elif linecount == 0 and self.linecount > 0: # dump empty table in to sql file
    ...
    def udpsend(self, sendstring = '', age = 0): # actual udp sending, no resend. give message as parameter. used by buff2server too.
        ...
        try:
            ''' SDP ei ole kattesaadav, sdp sisu tyhjendad vaja aga ei saa '''
            #try:
            if SDP:
                log.info('going to sdp.decode '+sendstring)
                try:
                    sdp = SDP(self.secret_key)
                    sdp.decode(sendstring)
                    sendstring = sdp.encode() # hash added as the last key:value pair
                    log.info('got from spd encoded '+sendstring)
                except:
                    log.warning('no signature added') # pass
                    traceback.print_exc()
            else:
                print('no signing possible due to missing SDP')
